...
// SignerExtractor provides a mock function with given fields:
func (_m *Lane) SignerExtractor() signer_extraction.Adapter {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for SignerExtractor")
	}

	var r0 signer_extraction.Adapter
	if rf, ok := ret.Get(0).(func() signer_extraction.Adapter); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(signer_extraction.Adapter)
	}
